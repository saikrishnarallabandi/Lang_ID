...
src_files_hinglish = os.listdir('../data/hinglish/cleaned')
...
for (fc,file) in enumerate(src_files_miami):
  if fc < limit:
    ...
    labels.append(1)
for (fc,file) in enumerate(src_files_hinglish):
  if fc < limit:
    files.append('../data/hinglish/cleaned/' + file)
    labels.append(1)
...
ae = ProbabilisticEncoderDecoderBiLSTM_file(m, units_input, units_hidden, units_output, units_latent, dy.rectify)
...
#      print count, " of ", c
      count += 1
      kl_loss, recons_loss = ae.calc_loss_basic(f,l)
  ...
  EER = threshold[np.argmin(abs(fnr-fpr))]
