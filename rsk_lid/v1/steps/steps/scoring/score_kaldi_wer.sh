...
word_ins_penalty=-1.0,-0.5,0.0,0.5,1.0,1.5,2.0
use_smbr=false
...
if [ $stage -le 0 ]; then
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    $use_smbr && ($cmd ACWT=$min_acwt:$max_acwt $dir/scoring/log/get_ctm.ACWT.log \
    ...
    (! $use_smbr) && ($cmd ACWT=$min_acwt:$max_acwt $dir/scoring/log/get_ctm.p${wip}.ACWT.log \
...
if [ $stage -le 1 ]; then
  ...
  for x in $dir/score_*/$name.ctm; do
    cp $x $d/tmpf;
    cat $d/tmpf | grep -i -v -E '\[NOISE|LAUGHTER|VOCALIZED-NOISE|SILENCE|silence\]' | \
#      grep -i -v -E '<UNK>' > $x;
      grep -i -v -E '<UNK>|%HESITATION|<HES>|<V-NOISE>' > $x;
    # hesitation is scored
  done
fi
...
if [ $stage -le 2 ]; then
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    $cmd ACWT=$min_acwt:$max_acwt $dir/scoring/log/score.p${wip}.ACWT.log \
      ...
      $hubscr -p $hubdir -V -l english -h rt-stt -g $data/glm -r $dir/score_p${wip}_ACWT/stm $dir/score_p${wip}_ACWT/${name}.ctm || exit 1;
  done
fi
  ...
  if [ $stage -le 3 ]; then
    for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
      $cmd ACWT=$min_acwt:$max_acwt $dir/scoring/log/score.swbd.p${wip}.ACWT.log \
        ...
        $hubscr -p $hubdir -V -l english -h rt-stt -g $data/glm -r $dir/score_p${wip}_ACWT/stm.swbd $dir/score_p${wip}_ACWT/${name}.ctm.swbd || exit 1;
    done
  fi
  ...
  if [ $stage -le 3 ]; then
    for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
      $cmd ACWT=$min_acwt:$max_acwt $dir/scoring/log/score.callhm.p${wip}.ACWT.log \
        ...
        $hubscr -p $hubdir -V -l english -h rt-stt -g $data/glm -r $dir/score_p${wip}_ACWT/stm.callhm $dir/score_p${wip}_ACWT/${name}.ctm.callhm || exit 1;
    done
  fi
 ;;
esac
